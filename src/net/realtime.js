...
// One Supabase Realtime channel per room, topic-namespaced with `hamnet:` — the
// project is shared with other games, so an unprefixed room code could collide
// with another game's channel topic.
const ROOM_PREFIX = 'hamnet:room:'
...
export function onOpenMenu(fn) { _handlers.open_menu = fn }
export function onCloseMenu(fn) { _handlers.close_menu = fn }
export function onGuestMenuAction(fn) { _handlers.guest_menu_action = fn }
export function onHostLeft(fn) { _handlers.host_left = fn }
export function onDisconnected(fn) { _handlers.disconnected = fn }

function testHook() {
  return typeof window !== 'undefined' ? window.__HAMNET_REALTIME_TEST_HOOK__ : null
}
...
export function sendGuestMenuAction(action) {
  _channel?.send({ type: 'broadcast', event: 'guest_menu_action', payload: { guestId: _myId, action } })
}

export function broadcastMenuOpenForGuest(guestId, buildingId) {
  _channel?.send({ type: 'broadcast', event: 'open_menu', payload: { guestId, buildingId: buildingId ?? null } })
}

export function broadcastMenuCloseForGuest(guestId) {
  _channel?.send({ type: 'broadcast', event: 'close_menu', payload: { guestId } })
}
