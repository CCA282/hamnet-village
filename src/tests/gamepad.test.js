...
describe('inputFor — pad source', () => {
  ...
  it('returns neutral input for an unclaimed player (source: null)', () => {
    const w = makePlayerCtx()
    const p = { id: 1, source: null, gamepadIndex: null }
    const input = makeInputCtx({ 0: makePad([0]) })
    expect(w.inputFor(input, p)).toEqual({ mx: 0, my: 0, action: false, actionHeld: false })
  })
})

// ── handleJoins — claiming an unclaimed player (restored from a save) ────────

describe('handleJoins — claims an unclaimed player instead of spawning a new one', () => {
  it('a gamepad press claims the first unclaimed (source: null) player', () => {
    const w = makePlayerCtx()
    const restored = { id: 1, source: null, gamepadIndex: null, color: 'red', label: 'Camille' }
    w.players = [restored]
    const input = makeInputCtx({ 0: makePad([0]) })
    w.handleJoins(input)
    expect(w.players).toHaveLength(1)
    expect(restored.source).toBe('pad')
    expect(restored.gamepadIndex).toBe(0)
    expect(restored.label).toBe('Camille') // identity preserved, only the device binding changes
  })

  it('a keyboard press claims the first unclaimed player', () => {
    const w = makePlayerCtx()
    const restored = { id: 1, source: null, gamepadIndex: null, color: 'red', label: 'Camille' }
    w.players = [restored]
    const input = {
      ...makeInputCtx({}),
      keyboardState: (scheme) => ({ action: scheme === 'kb1', actionHeld: false, mx: 0, my: 0, cancel: false }),
    }
    w.handleJoins(input)
    expect(w.players).toHaveLength(1)
    expect(restored.source).toBe('kb1')
  })

  it('falls back to spawning a new player when nobody is unclaimed', () => {
    const w = makePlayerCtx()
    w.addPlayer('kb1')
    const input = makeInputCtx({ 0: makePad([0]) })
    w.handleJoins(input)
  ...
  it('only claims one unclaimed player per join, leaving the rest for later devices', () => {
    const w = makePlayerCtx()
    const r1 = { id: 1, source: null, gamepadIndex: null, color: 'red', label: 'P1' }
    const r2 = { id: 2, source: null, gamepadIndex: null, color: 'blue', label: 'P2' }
    w.players = [r1, r2]
    const input = makeInputCtx({ 0: makePad([0]) })
    w.handleJoins(input)
    expect(r1.source).toBe('pad')
